...
void	init_window(t_mlx *g_mlx, t_map_size *map_info, t_data *data)
{
	...
	g_mlx->win_ptr = mlx_new_window(g_mlx->mlx_ptr, WIN_WIDTH,
			WIN_HEIGHT, "Cub3D");
...
void	draw_mlx_map(t_vars *vars, t_mlx *g_mlx, t_map_size *map_info)
{
	...
	while (i < map_info->num_of_lines)
	{
		...
		while (j < ft_strlen(map_info->map[i]))
		{
			...
			{
				draw_rect(g_mlx, j * TILE_SIZE / 5, i * TILE_SIZE / 5,
					TILE_SIZE / 5, TILE_SIZE / 5, 0x00FF00);
			}
			j++;
		}
		i++;
	}
		draw_rect(g_mlx, vars->player->pixel_x / 5 - 5,
				vars->player->pixel_y / 5 - 5,TILE_SIZE / 5,TILE_SIZE / 5, 0xFFFF00);
}
