...
void img_to_xpm(t_vars *vars)
{
    ...
    vars->mlx->n_xpm = mlx_xpm_file_to_image(vars->mlx->mlx_ptr, vars->mlx->north_img, &width, &height);
    vars->mlx->s_xpm = mlx_xpm_file_to_image(vars->mlx->mlx_ptr, vars->mlx->south_img, &width, &height);
    vars->mlx->e_xpm = mlx_xpm_file_to_image(vars->mlx->mlx_ptr, vars->mlx->east_img, &width, &height);
    vars->mlx->w_xpm = mlx_xpm_file_to_image(vars->mlx->mlx_ptr, vars->mlx->west_img, &width, &height);
    if (!vars->mlx->n_xpm || !vars->mlx->s_xpm || !vars->mlx->w_xpm || !vars->mlx->e_xpm)
    {
        printf("Error Loading Textures\n");
        exit(0);
    }
    vars->mlx->addr_n = mlx_get_data_addr(vars->mlx->n_xpm, &vars->mlx->bpp_n, &vars->mlx->l_len_n, &vars->mlx->endian_n);
    vars->mlx->addr_s = mlx_get_data_addr(vars->mlx->s_xpm, &vars->mlx->bpp_s, &vars->mlx->l_len_s, &vars->mlx->endian_s);
    vars->mlx->addr_w = mlx_get_data_addr(vars->mlx->w_xpm, &vars->mlx->bpp_w, &vars->mlx->l_len_w, &vars->mlx->endian_w);
    vars->mlx->addr_e = mlx_get_data_addr(vars->mlx->e_xpm, &vars->mlx->bpp_e, &vars->mlx->l_len_e, &vars->mlx->endian_e);
}
char *north_textures(t_vars *vars, int i)
{
    ...
    if (vars->mlx->wall_height > WIN_HEIGHT)
        y = (int)(round)((i + (vars->mlx->wall_height - WIN_HEIGHT) / 2.0) * (1000.0 / vars->mlx->wall_height)) % 1000;   
   else
